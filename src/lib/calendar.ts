...
SUMMARY:${title}
LOCATION:${location}
DESCRIPTION:Join us for the Irish Roadshow event in ${location}.\\n\\n• Check your email for confirmation\\n• Join the WhatsApp group using the link in the confirmation email\\n• Add the event to your calendar
END:VEVENT
END:VCALENDAR`;
...
export function generateOutlookCalendarUrl(details: CalendarDetails): string {
  ...
  const params = new URLSearchParams({
    ...
    location: location,
    body: `Join us for the Irish Roadshow event in ${location}.\n\n• Check your email for confirmation\n• Join the WhatsApp group using the link in the confirmation email\n• Add the event to your calendar`,
  });
